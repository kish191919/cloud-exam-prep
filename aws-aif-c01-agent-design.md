...
에이전트: 안녕하세요! AWS 시험 문제 변환을 시작합니다.
  exam_id를 입력해주세요. (예: aws-aif-c01, aws-clf-c02, aws-dea-c01, aws-saa-c03, azure-az-900)
...
### 품질 자기검증 체크리스트 (STEP 5 — 10개 항목)

LLM이 자기검증 시 다음 10개 항목을 순서대로 확인한다:

| # | 항목 | 검증 방법 | 실패 시 처리 |
|---|------|----------|------------|
| 1 | **정답 논리 유효성** | 정답 보기가 명확히 옳은가? 근거가 있는가? | 재시도 (최대 2회) |
| 2 | **오답 그럴듯함** | 각 오답이 수험생이 실제로 고민할 만큼 그럴듯한가? | 재시도 (최대 2회) |
| 3 | **한국어 자연스러움** | 번역투 표현 없이 자연스러운 한국어인가? | 재시도 (최대 2회) |
| 4 | **원문 AWS 서비스명 보존** | 원문에 등장한 서비스명이 재설계 후에도 동일하게 사용되는가? | 재시도 (최대 2회) |
| 5 | **보기 길이·형식** | 40자 이내, 4개 길이 균일, 파이프(`|`) 구분 금지, 서비스 3개+ 나열 금지 | 재시도 (최대 2회) |
| 6 | **보기 중복 없음** | 4개 보기가 각각 서로 다른 서비스·접근법인가? | 재시도 (최대 2회) |
| 7 | **질문 명확성** | 모호함 없이 정답이 단 하나인가? | 재시도 (최대 2회) |
| 8 | **단일 개념 집중** | 하나의 핵심 판단만 테스트하는가? | 재시도 (최대 2회) |
| 9 | **규칙 12: 정답 위치 원문과 다름** | 재설계 `correct_option_id`가 원문 `answer` 위치와 다른가? | 재시도 (최대 2회) |
| 10 | **원본 정답 AWS 기술적 유효성** | `correct_answer_concept`이 원문 보기 전체 대비 AWS 공식 기능 기준으로 명확히 가장 적합한 답인가? | ⚠️ 즉시 에스컬레이션 |

**판정:**
- 1~9번 실패: 해당 항목만 수정 후 재설계 재시도 (최대 2회) → 에스컬레이션
- **10번 실패: 재시도 없이 즉시 에스컬레이션** — 사용자에게 [A] 정답 개념 변경 후 재설계 / [B] 원본 신뢰 후 진행 / [C] 스킵 선택 요청
