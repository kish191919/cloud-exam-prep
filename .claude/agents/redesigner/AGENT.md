...
- `tag_en`, `tag_pt`, `tag_es`, `tag_ja`는 STEP 5.5에서 domain_tags 표의 해당 열에서 복사
...
**규칙 5a (보기 해설 필수)** ⚠️ 정답 1개 + 오답 3개, **4개 모든 보기**에 `explanation` 필드와 모든 언어 번역(`explanation_en/pt/es/ja`) 필수. 빈 문자열(`""`) 또는 생략 절대 금지.
...
모든 언어 버전(`explanation_en/pt/es/ja`)도 동일하게 헤더 없는 3단락 자유형으로 작성한다.
...
```json
{
  ...
  "explanation_ja": "{段落1: 問題解法アプローチと正解の根拠}\n\n{段落2: 概念説明と実践的文脈}\n\n{段落3: 試験のポイント}",
  "key_points": "핵심 개념 제목\n• 포인트 1\n• 포인트 2\n• 포인트 3",
...
[PASS] 다국어 줄바꿈 서식: text_en/pt/es/ja 필드에도 질문 앞 \n\n 확인, 4문장+ 시 첫 문장 뒤 \n\n 확인
...
[PASS] tag 한국어 저장: tag 필드가 domain_tags 첫 번째 열(한국어) 값과 정확히 일치 — 영문·일문 등 비한국어 → FAIL
[PASS] text_* 완전 번역: text_en/text_pt/text_es/text_ja 4개 필드 모두 비어있지 않고 각 해당 언어(비한국어)로 작성됨 — NULL·빈 문자열·한국어 원문 잔류 → FAIL
...
[PASS] 다국어 번역 완결성: explanation_en/pt/es/ja, key_points_en/pt/es/ja, options[].explanation_en/pt/es/ja 모두 해당 언어로 작성됨 (한글(가-힣) 포함 → 즉시 FAIL)
```
...
- **하나라도 FAIL** → 해당 항목만 수정 후 재검증 (최대 2회)
  ...
  - 다국어 번역 완결성 FAIL: 한글이 포함된 `_en/_pt/_es/_ja` 필드를 해당 언어로 즉시 재번역 후 재검증
