...
7. **언어 순수성 (절대 규칙)**: 각 필드 접미사별 사용 언어는 다음과 같이 고정된다:
   ...
   - 접미사 없는 필드(`text`, `explanation`, `key_points`, `options[].text`, `options[].explanation`, `tag`) → 반드시 **한국어**로만 작성. 영어·일본어 절대 금지
   - AWS 서비스명(Amazon Bedrock, SageMaker 등)만 모든 언어에서 원문 유지
8. **번역 자기검증**: 각 문제 번역 완료 후 다음을 확인한다:
   - `text_ja`에 한글(ㄱ-ㅎ, 가-힣)이 포함되어 있으면 → 일본어로 재번역
   - `text`(한국어)에 일본어 문자(ひ, カ 등)가 포함되어 있으면 → 한국어로 재작성
   - `text_en`에 한글/일본어가 포함되어 있으면 → 영어로 재번역
