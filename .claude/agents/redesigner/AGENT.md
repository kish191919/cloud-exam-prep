...
```json
{
  ...
  "tag_pt": "Tag de domínio (Português — domain_tags 테이블 포르투갈어 태그 열에서 복사)",
  "tag_es": "Etiqueta de dominio (Español — domain_tags 테이블 스페인어 태그 열에서 복사)",
  "tag_ja": "ドメインタグ（日本語 — domain_tags テーブルの日本語タグ列からコピー）"
}
```
