...
[PASS] 보기 해설 완결성: options 4개 모두 explanation + explanation_en 필드 포함
[PASS] 참고자료: ref_links 1~3개의 유효한 docs.aws.amazon.com / aws.amazon.com URL 포함
[PASS] 언어 순수성: text 및 options[].text에 일본어 문자(ひらがな, カタカナ)와 AWS 서비스명 외 영어 문장이 없음
[PASS] options 완결성: a, b, c, d 4개 보기 모두 text + explanation 필드가 누락 없이 포함됨
[PASS] 영문 언어 순수성: text_en 및 options[].text_en에 한국어·일본어 등 영어 외 문자가 없음
...
- **1~12번, 15~20번 중 하나라도 FAIL** → 해당 항목만 수정 후 재검증 (최대 2회)
  ...
  - **17번(언어 순수성) FAIL 시:** 일본어·영어 문장을 한국어로 교체 후 재검증
