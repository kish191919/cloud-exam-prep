...
-- Create junction table: exam_sets <-> questions (many-to-many)
...
CREATE INDEX IF NOT EXISTS idx_exam_set_questions_set_id ON exam_set_questions(set_id);
CREATE INDEX IF NOT EXISTS idx_exam_set_questions_question_id ON exam_set_questions(question_id);

-- RLS
ALTER TABLE exam_sets ENABLE ROW LEVEL SECURITY;
ALTER TABLE exam_set_questions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public read access to exam_sets"
  ON exam_sets FOR SELECT USING (true);

CREATE POLICY "Allow public read access to exam_set_questions"
...
-- Create view: exam_sets with question count
