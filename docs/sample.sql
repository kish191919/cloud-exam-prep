...
-- ⚠️  key_point_images 컬럼 제외 (DB에 존재하지 않음)
...
INSERT INTO question_options (question_id, option_id, text, explanation, sort_order) VALUES
  ('awsaifc01-q166', 'a', 'Amazon Transcribe', '음성을 텍스트로 변환하는 STT 서비스입니다. 문서 이미지나 PDF에서 텍스트를 추출하는 용도로는 사용할 수 없습니다.', 1),
  ('awsaifc01-q166', 'b', 'Amazon Textract', '인쇄 텍스트·표·양식뿐 아니라 손글씨까지 문서에서 자동 추출하는 ML 서비스로, 이 요구사항에 가장 적합합니다.', 2),
  ('awsaifc01-q166', 'c', 'Amazon Comprehend', '텍스트 분석(감정·개체·토픽) NLP 서비스입니다. 이미 추출된 텍스트를 분석하는 데 사용하며, 문서 이미지에서 직접 추출하지는 않습니다.', 3),
  ('awsaifc01-q166', 'd', 'Amazon Rekognition', '이미지·동영상에서 객체·얼굴·장면을 인식하는 서비스입니다. 문서 내 텍스트 데이터 추출에는 Textract가 적합합니다.', 4);
...
INSERT INTO question_options (question_id, option_id, text, explanation, sort_order) VALUES
  ('awsaifc01-q167', 'a', 'Amazon Polly + Amazon Rekognition', 'Polly는 TTS(텍스트→음성) 서비스이고 Rekognition은 이미지 인식 서비스로, 이 요구사항의 어떤 단계도 처리하지 못합니다.', 1),
  ('awsaifc01-q167', 'b', 'Amazon Transcribe + Amazon Comprehend', 'Transcribe로 음성을 텍스트로 변환하고, Comprehend로 감정을 분석합니다. 이 요구사항에 정확히 맞는 조합입니다.', 2),
  ('awsaifc01-q167', 'c', 'Amazon Lex + Amazon Translate', 'Lex는 대화형 챗봇 서비스이고 Translate는 언어 번역 서비스입니다. 감정 분석 기능이 없습니다.', 3),
  ('awsaifc01-q167', 'd', 'Amazon Textract + Amazon Kendra', 'Textract는 문서 이미지 텍스트 추출, Kendra는 지능형 문서 검색 서비스입니다. 음성 처리와 감정 분석에는 적합하지 않습니다.', 4);
